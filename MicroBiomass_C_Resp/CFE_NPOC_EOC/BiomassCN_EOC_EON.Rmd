...
#check column names
  ...
  calcs <- input %>%
    ...
    mutate(
      NPOC = (((NPOC - NPOC_blank) * (subsample_vol + dilution_vol)) * (K2SO4_vol / 1000)) / 
        (soil_weight_extracted * (1 - fwc) *1000),
      NPOC_cfe = (((NPOC_cfe - NPOC_cfe_blank) * (subsample_vol + dilution_vol)) * 
        (K2SO4_vol / 1000)) / (soil_weight_extracted_cfe * (1 - fwc) *1000),
      TN = (((TN - TN_blank) * (subsample_vol + dilution_vol)) * (K2SO4_vol / 1000)) / 
        (soil_weight_extracted * (1 - fwc) *1000),
      TN_cfe = (((TN_cfe - TN_cfe_blank) * (subsample_vol + dilution_vol)) * 
        (K2SO4_vol / 1000)) / (soil_weight_extracted_cfe * (1 - fwc) *1000),
      #then calculate microbial biomass C & N, Exchangeable organic C & N 
      micC = (NPOC_cfe - NPOC) / micC_corr,
      micN = (TN_cfe - TN) / micN_corr,
      micCN = micC/micN,
      EOC = NPOC,
      EON = TN - TIN
    )
